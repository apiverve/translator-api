...
class TranslatorAPIClientError(Exception):
...
class ValidationError(TranslatorAPIClientError):
...
class TranslatorAPIClient:
    ...
    VALIDATION_RULES = {"text": {"type": "string", "required": True}, "target": {"type": "string", "required": True}, "source": {"type": "string", "required": False}}
    ...
    def __init__(self, api_key: str, secure: bool = True, debug: bool = False):
        """
        Initialize the TranslatorAPIClient with the API key and optional settings

        :param api_key: Your APIVerve API key
        :param secure: Deprecated. Always set to True
        :param debug: Enable debug logging (default: False)
        :raises TranslatorAPIClientError: If API key is invalid
        """
    ...
    def _validate_api_key(self, api_key: str) -> None:
        ...
        if not api_key or not api_key.strip():
            raise TranslatorAPIClientError(
    ...
    def _log(self, message: str) -> None:
        ...
        """
        if self.debug:
            print(f"[TranslatorAPIClient DEBUG] {message}")
    ...
    def execute(self, params: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:  # Returns TranslatorData in 'data' field
        """
        Execute the API request

        :param params: API parameters (optional)
        :return: API response as a dictionary
        :raises TranslatorAPIClientError: If the request fails or returns an error
        ...
        try:
            ...
            raise TranslatorAPIClientError(f"Request timeout: {str(e)}")
        except requests.exceptions.ConnectionError as e:
            self._log(f"Connection error: {str(e)}")
            raise TranslatorAPIClientError(f"Connection error: {str(e)}")
        except requests.exceptions.RequestException as e:
            self._log(f"Request exception: {str(e)}")
            raise TranslatorAPIClientError(f"Request failed: {str(e)}")
